...
class Layer_Dense:
    ...
    def __init__(self, n_inputs, n_neurons,
        ...
        if to_Tensors:
            ...
            self.weight_regularizer_l1 = torch.tensor(weight_regularizer_l1)
            self.weight_regularizer_l2 = torch.tensor(weight_regularizer_l2)
            self.bias_regularizer_l1 = torch.tensor(bias_regularizer_l1)
            self.bias_regularizer_l2 = torch.tensor(bias_regularizer_l2)

            self.weight_regularizer_l1 = self.weight_regularizer_l1.to("cuda:0")
            self.weight_regularizer_l2 = self.weight_regularizer_l1.to("cuda:0")
            self.bias_regularizer_l1 = self.bias_regularizer_l1.to("cuda:0")
            self.bias_regularizer_l2 = self.bias_regularizer_l2.to("cuda:0")
        else:
    ...
    def backward(self, dvalues):
        ...
        pack = [lambda: np, lambda: torch][to_Tensors]()

        if to_Tensors:
            temp = torch.transpose(self.inputs, 0, 1)
            temp2 = torch.mm(temp.type(torch.DoubleTensor), dvalues.type(torch.DoubleTensor))
            self.dweights = temp2
            self.dweights = self.dweights.to("cuda:0")
        else:
            self.dweights = np.dot(self.inputs.T, dvalues)
        self.dbiases = pack.sum(dvalues, axis=0, keepdims=True)

        # Gradients on regularization
        # L1 on weights
        if self.weight_regularizer_l1 > 0:
            dL1 = pack.ones_like(self.weights)
        ...
        if self.bias_regularizer_l1 > 0:
            dL1 = pack.ones_like(self.biases)
            dL1[self.biases < 0] = -1
        ...
        # Gradient on values
        if to_Tensors:
            self.dinputs = torch.mm(dvalues, torch.transpose(self.weights, 0, 1))
        else:
...
class Layer_Dropout:
    ...
    def forward(self, inputs, training):
        ...
        if to_Tensors:
            print("Not finished for torch")
            self.binary_mask = torch.binomial(1, self.rate)
        else:
...
class Accuracy_Regression(Accuracy):
    ...
    def init(self, y, reinit=False):
        ...
        if self.precision is None or reinit:
            self.precision = pack.std(y) / 250

    # Compares predictions to the ground truth values
    def compare(self, predictions, y):
        pack = [lambda: np, lambda: torch][to_Tensors]()
        return pack.absolute(predictions - y) < self.precision
...
class Activation_Softmax:
    ...
    def backward(self, dvalues):
        ...
        self.dinputs = pack.empty_like(dvalues)
...
class Activation_Sigmoid:
    ...
    def forward(self, inputs, training):
        ...
        self.inputs = inputs
        self.output = 1 / (1 + pack.exp(-inputs))
    ...
    def update_params(self, layer):
        ...
        if self.momentum:
            ...
            if not hasattr(layer, 'weight_momentums'):
                layer.weight_momentums = pack.zeros_like(layer.weights)
                # if there is no momentum array for weights
                # the array doesn't exist for biases yet either
                layer.bias_momentums = pack.zeros_like(layer.biases)
...
class Optimizer_Adagrad:
    ...
    def update_params(self, layer):
        ...
        if not hasattr(layer, 'weight_cache'):
            layer.weight_cache = pack.zeros_like(layer.weights)
            # if there is no cache array for weights
            # the array doesn't exist for biases yet either
            layer.bias_cache = pack.zeros_like(layer.biases)
        ...
        layer.weights += -self.current_learning_rate * \
            layer.dweights / \
            (pack.sqrt(layer.weight_cache) + self.epsilon)
        layer.biases += -self.current_learning_rate * \
            layer.dbiases / \
            (pack.sqrt(layer.bias_cache) + self.epsilon)
...
class Optimizer_Adam:
    ...
    def update_params(self, layer):
        ...
        layer.weights += -self.current_learning_rate * \
                        ...
                        (pack.sqrt(weight_cache_corrected) + self.epsilon)
        layer.biases += -self.current_learning_rate * \
                        bias_momentums_corrected / \
                        (pack.sqrt(bias_cache_corrected) + self.epsilon)
...
class Loss:
        ...
        for layer in self.trainable_layers:
            ...
            if layer.weight_regularizer_l1 > 0:
                regularization_loss += layer.weight_regularizer_l1 * \
                                        pack.sum(pack.abs(layer.weights))

            # L2 regularization - weights
            if layer.weight_regularizer_l2 > 0:
                regularization_loss += layer.weight_regularizer_l2 * \
                                        pack.sum(layer.weights * layer.weights)

            # L1 regularization - biases
            # calculate only when factor greater than 0
            if layer.bias_regularizer_l1 > 0:
                regularization_loss += layer.bias_regularizer_l1 * \
                                        pack.sum(pack.abs(layer.biases))

            # L2 regularization - biases
            if layer.bias_regularizer_l2 > 0:
                regularization_loss += layer.bias_regularizer_l2 * \
                                        pack.sum(layer.biases * layer.biases)
    ...
    def calculate(self, output, y, *, include_regularization=False):
        ...
        if to_Tensors:
            ...
            self.accumulated_sum += torch.sum(sample_losses)
...
class Loss_CategoricalCrossentropy(Loss):
    ...
    def forward(self, y_pred, y_true):
        ...
        y_pred_clipped = pack.clip(y_pred, 1e-7, 1 - 1e-7)
        ...
        # Mask values - only for one-hot encoded labels
            ...
            correct_confidences = pack.sum(y_pred_clipped*y_true, axis=1)
        ...
        # If not any of these two
            ...
            exit()
        # Losses
        negative_log_likelihoods = -pack.log(correct_confidences)
...
class Loss_BinaryCrossEntropy(Loss):
    ...
    def forward(self, y_pred, y_true):
        ...
        sample_losses = -(y_true * pack.log(y_pred_clipped) +
                          (1 - y_true) * pack.log(1 - y_pred_clipped))
        sample_losses = pack.mean(sample_losses, axis=-1)
    ...
    def backward(self, dvalues, y_true):
        ...
        # Clip data to prevent division by 0
        # Clip both sides to not drag mean towards any value
        clipped_dvalues = pack.clip(dvalues, 1e-7, 1 - 1e-7)
...
class Loss_MeanSquaredError(Loss):   # L2 loss
    ...
    def forward(self, y_pred, y_true):
        ...
        pack = [lambda: np, lambda: torch][to_Tensors]()
        # Calculate loss
        sample_losses = pack.mean((y_true - y_pred)**2, axis=-1)
        # Return losses
        return sample_losses
...
class Loss_MeanAbsoluteError(Loss):
    ...
    def forward(self, y_pred, y_true):
        ...
        pack = [lambda: np, lambda: torch][to_Tensors]()
        # Calculate loss
        sample_losses = pack.mean(pack.abs(y_true - y_pred), axis=-1)
    ...
    def backward(self, dvalues, y_true):
        ...
        # Calculate gradient
        self.dinputs = pack.sign(y_true - dvalues) / outputs
        # Normalize gradient
        self.dinputs = self.dinputs / samples
